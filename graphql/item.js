...
const resolvers = {
    ...
    items: async(parent, {organization, subCategory, search, sort}, {user}) => {
        if(['admin', 'суперагент', 'экспедитор', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)){
            ...
            if(user.organization)
                organizations = [user.organization, ...(await Distributer.findOne({
    ...
    brands: async(parent, {organization, search, sort, city}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(organization)) {
            let subBrand = await SubBrand.findOne({_id: organization}).select('organization _id').lean()
            if(subBrand){
...
const resolversMutation = {
    ...
    setItemsCostPrice: async(parent, { itemsCostPrice }, {user}) => {
        if(user.role==='admin') {
            ...
            for (let i = 0; i < itemsCostPrice.length; i++) {
                object = await Item.findOne({_id: itemsCostPrice[i]._id})
    ...
    onoffItem: async(parent, { _id }, {user}) => {
        let objects = await Item.find({_id: {$in: _id}})
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'|| (['суперорганизация', 'организация'].includes(user.role)&&user.organization.toString()===objects[i].organization.toString())){
                ...
                await Basket.deleteMany({item: {$in: objects[i]._id}})
            }
        }
        return {data: 'OK'}
    },
    deleteItem: async(parent, { _id }, {user}) => {
        if(['admin', 'суперорганизация', 'организация'].includes(user.role)) {
            await Item.updateMany({_id: {$in: _id}}, {
                del: 'deleted',
                status: 'deactive'
            })
            let objects = await Item.find({_id: {$in: _id}, ...user.organization?{organization: user.organization}:{}})
            for(let i=0; i<objects.length; i++){
            ...
            objects = await Ads.find({item: {$in: _id}}).select('image').lean()
            for (let i = 0; i < objects.length; i++) {
                await deleteFile(objects[i].image)
            }
            await Ads.updateMany({_id: {$in: _id}}, {del: 'deleted'})
            await Basket.deleteMany({item: {$in: _id}})
            await Integrate1C.deleteMany({item: {$in: _id}})
        }
        return {data: 'OK'}
    },
    restoreItem: async(parent, { _id }, {user}) => {
        if(user.role==='admin') {
            await Item.updateMany({_id: {$in: _id}}, {del: null, status: 'active'})
        }
        return {data: 'OK'}
    }
};
