            ...
            for(let i=0; i<findRes.length; i++){
                ...
                if(findRes[i].user){
                    if('admin'===findRes[i].user.role) {
                        ...
                        let client = await Client.findOne({user: findRes[i].user._id}).select('name address').lean()
                        if(client)
                        ...
                        let employment = await Employment.findOne({user: findRes[i].user._id}).lean()
                        if(employment)
                            res[i].user = `${findRes[i].user.role} ${employment.name}`
                    }
                    else {
                        let employment = await Employment.findOne({user: findRes[i].user._id}).populate({ path: 'organization' }).lean()
                        if(employment)
...
const resolversMutation = {
    deleteSubscriber: async(parent, { _id }, {user}) => {
        if('admin'===user.role){
            await Subscriber.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    }
};
