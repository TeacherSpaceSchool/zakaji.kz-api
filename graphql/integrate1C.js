...
const resolvers = {
    integrate1CsSimpleStatistic: async(parent, {search, filter, organization}, {user}) => {
        if(user.role==='admin'){
            ...
            let integrate1Cs =  await Integrate1C.aggregate(
                [
                    {
                        $match:{
    ...
    ecspeditorsIntegrate1C: async(parent, {organization}, {user}) => {
        if(user.role==='admin') {
            let ecspeditors =  await Integrate1C
                .find({
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    ecspeditor: {$ne: null}
                })
                .distinct('ecspeditor').lean()
            ecspeditors = await Employment.find({
                ...(organization==='super'?{organization: null}:{organization: organization}),
                _id: {$nin: ecspeditors},
                del: {$ne: 'deleted'}
            })
    ...
    clientsIntegrate1C: async(parent, {organization, city}, {user}) => {
        if(user.role==='admin') {
            ...
            if(organization!=='super')
                organization = await Organization.findOne({_id: organization}).lean()
            if(organization.accessToClient||organization==='super') {
                ...
                let clients1 = await Order.find({item: {$in: items}}).distinct('client').lean()
                clients = await Client.find({
                    $and: [
                        {_id: {$in: clients1}},
                        {_id: { $nin: clients}}
...
const resolversMutation = {
    ...
    setIntegrate1C: async(parent, {_id, item, client, guid, agent, ecspeditor}, {user}) => {
        if(user.role==='admin') {
            let object = await Integrate1C.findById(_id)
    ...
    deleteIntegrate1C: async(parent, { _id }, {user}) => {
        if(user.role==='admin')
            await Integrate1C.deleteMany({_id: {$in: _id}})
        return {data: 'OK'}
    },
        ...
        if(user.role==='admin'){
            ...
            for(let i = 0;i<rows.length;i++){
                if(mongoose.Types.ObjectId.isValid(rows[i][0])&&rows[i][1]&&rows[i][1].length>0) {
                    let client = await Client.findOne({_id: rows[i][0]}).select('_id').lean()
                    if (client) {
                        ...
                        if(integrate1C) {
                            integrate1C.guid = rows[i][1]
                            await integrate1C.save()
                        }
                        else {
                            let _object = new Integrate1C({
                                item: null,
