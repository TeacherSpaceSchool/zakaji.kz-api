...
const checkAdss = async(invoice) => {
    invoice = await Invoice.findOne({_id: invoice})
...
const resolvers = {
    ...
    adss: async(parent, {search, organization}, {user}) => {
        if(user.role) {
            let _organization = await SubBrand.findOne({_id: organization}).select('organization').lean()
            let res = await Ads.find({
                del: {$ne: 'deleted'},
                title: {'$regex': search, '$options': 'i'},
    ...
    adsOrganizations: async(parent, ctx, {user}) => {
        if(user.role) {
            if (user.organization) {
                let distributer = await Distributer.findOne({distributer: user.organization})
                ...
                if (distributer) {
                    ...
                    distributer = await Organization
                ...
                let organizations = await Ads.find({del: {$ne: 'deleted'}}).distinct('organization').lean()
    ...
    ads: async(parent, ctx, {user}) => {
        if(user.role) {
            let ads = await Ads.findRandom().limit(1).lean()
            return ads[0]
        }
    }
};
...
const resolversMutation = {
    ...
    restoreAds: async(parent, { _id }, {user}) => {
        if('admin'===user.role){
            await Ads.updateMany({_id: {$in: _id}}, {del: null})
        }
        return {data: 'OK'}
    },
    deleteAds: async(parent, { _id }, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)){
            let objects = await Ads.find({_id: {$in: _id}}).select('image').lean()
