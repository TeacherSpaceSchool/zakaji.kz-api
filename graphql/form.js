const TemplateForm = require('../models/form/templateForm');
...
const resolvers = {
    templateForms: async(parent, {search, organization, skip}, {user}) => {
        if(['client', 'агент', 'admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)){
            let templateForms
            if(user.role==='client')
                templateForms = await Form.find({client: user.client}).distinct('templateForm').lean()
            return await TemplateForm.find({
    ...
    analysisForms: async(parent, {templateForm}, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)) {
            let forms = await Form.find({...user.organization?{organization: user.organization}:{}, templateForm: templateForm})
    ...
    templateForm: async(parent, {_id}, {user}) => {
        if(['суперорганизация', 'организация', 'менеджер', 'агент', 'admin', 'client'].includes(user.role)){
            return await TemplateForm.findOne({
                _id: _id,
    ...
    forms: async(parent, {templateForm, search, skip}, {user}) => {
        if(['агент', 'суперорганизация', 'организация', 'менеджер', 'admin'].includes(user.role)){
            ...
            if(['агент', 'менеджер'].includes(user.role)){
                districtClients = await District
                    .find({$or: [{manager: user.employment}, {agent: user.employment}]})
                    .distinct('client')
                if(user.onlyIntegrate){
                    districtClients = await Integrate1C
                        .find({client: {$in: districtClients}, organization: user.organization})
                        .distinct('client')
                        .lean()
                }
                templateForm = (await TemplateForm.findOne({_id: templateForm, organization: user.organization, editorEmployment: true}).select('_id').lean())._id
            }
            let _clients;
            if(search.length>0) {
...
const resolversMutation = {
    addTemplateForm: async(parent, {title, organization, editorEmployment, editorClient, edit, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            let newTemplateForm = new TemplateForm({
                title: title,
            ...
            await TemplateForm.create(newTemplateForm);
        }
        return {data: 'OK'}
    },
    setTemplateForm: async(parent, {_id, title, editorEmployment, editorClient, edit, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            let templateForm = await TemplateForm.findOne({_id: _id, ...user.organization?{organization: user.organization}:{}})
            if(templateForm){
    ...
    addForm: async(parent, {templateForm, client, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)) {
            templateForm = await TemplateForm.findOne({_id: templateForm, ...user.organization?{organization: user.organization}:{}}).select('organization _id').lean()
    ...
    setForm: async(parent, {_id, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)) {
            let form = await Form.findOne({_id: _id, ...user.organization?{organization: user.organization}:{}})
            if(form) {
                let templateForm = await TemplateForm.findOne({_id: form.templateForm}).select('edit').lean()
    ...
    deleteTemplateForm: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            await TemplateForm.deleteMany({_id: {$in: _id}})
            await Form.deleteMany({templateForm: {$in: _id}})
        }
        return {data: 'OK'}
    },
    deleteForm: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент'].includes(user.role)) {
            await Form.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    },
