...
module.exports.reductionToEmployment = async() => {
    ...
    for(let i=0; i<createdEmployment.length; i++) {
        employment = await Employment.findOne({email: createdEmployment[i].email}).lean()
        if(!employment) {
            employment = new Employment({...createdEmployment[i], del: 'deleted'});
            employment = await Employment.create(employment);
            count = await Invoice.countDocuments({agent: new mongoose.Types.ObjectId(employment.email)})
            await Invoice.updateMany({agent: new mongoose.Types.ObjectId(employment.email)}, {agent: employment._id})
    ...
    for(let i=0; i<deletedEmployment.length; i++) {
        employment = await Employment.findOne({_id: deletedEmployment[i]}).lean()
        if(employment)
